						...
						<div class="bg-white rounded30 shadow-lg">
							...
							<?php foreach ($errors->all() as $message) {?>
										<p style="color:Red"><?php echo $message?></p>
															...
															@endif
								<form action="{{route('editCategoryPost',[$category->id])}}" method="post" enctype="multipart/form-data">
									@csrf
                                 ...
                                 role="tabpanel" aria-labelledby="pills-tab-{{ $locale->code }}">
									<div class="form-group">
										<div class="input-group mb-3 ">
											...
											<input id="name"  value="{{$category->setLocale($locale->code)->name}}" required name="{{$locale->code}}" type="" class="form-control pl-15 bg-transparent" placeholder="{{$locale->name}}">
										</div>
										</div>
										</div>
										@endforeach
									...
									<div class="form-group">
										<div class="input-group mb-3">
											...
											<select value="" class="form-control pl-15 bg-transparent" name="status" id="" value="">
												<option class="form-control pl-15 bg-transparent" value="active" {{($category->status == 'active') ? 'selected': '' }} >active</option>
												<option class="form-control pl-15 bg-transparent"  value="inactive" {{ ($category->status == 'inactive') ? 'selected': '' }} >inactive</option>
											</select>
